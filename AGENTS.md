...
- A normal stop/start or process restart of the same VPN container retains its container ID but changes its start timestamp. The orchestrator uses both values to detect the restart and reconcile every dependent.
...
- `vpn-orchestrator` polls the VPN container ID, start timestamp, and dependent health every 15 seconds. On its own startup it immediately reconciles only missing or unhealthy dependents when the recorded VPN generation is unchanged, covering NAS restarts without relying solely on Docker events.
- A changed VPN container ID or start timestamp triggers full parallel stop/start reconciliation. An unhealthy dependent with an unchanged VPN generation gets a 60-second maintenance grace period and then targeted reconciliation only if it remains unhealthy. This prevents an intentional single-stack redeployment from restarting every VPN-dependent service.
...
- The orchestrator records the new VPN generation only after every dependent is healthy, so a partial failure is retried.
- VPN generation and heartbeat state live under `/Volume1/public/docker-configs/vpn-orchestrator`. The generation combines the container ID and start timestamp. The API key is supplied by the `PORTAINER_API_KEY` variable configured on the Portainer stack; never commit its value.
